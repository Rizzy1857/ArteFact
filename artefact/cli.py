...
def list_tools():
    table = Table(title="Available Tools in ArteFact v0.4.0a", show_lines=True)
    ...
    table.add_row("meta", "Extract metadata from images, PDFs, and more")
    table.add_row("timeline", "Generate a forensics timeline from file timestamps")
    table.add_row("memory", "Analyze memory dumps for strings, processes, and indicators")
    table.add_row("mount", "Mount and parse forensic disk images (E01, AFF, etc.)")
